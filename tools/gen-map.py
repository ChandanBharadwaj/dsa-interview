...
CUES = {
    ...
    '07:lc69':  'Last k with k² ≤ x — answer-space search; cast to long before squaring.',
    '07:lc74':  'Row-chained matrix = one sorted list; search the flat index.',
    '07:lc162': 'No sortedness needed — the slope at mid is the direction oracle.',
    '07:lc540': 'Search where pair-start index parity flips from even to odd.',
    '07:lc410': 'Minimize-the-max ⇒ binary search the cap + greedy feasibility.',
    ...
    '08:lc83':  'Sorted = duplicates adjacent; splice without advancing.',
    '08:lc234': 'Middle + reverse second half + lockstep compare — three skills chained.',
    '08:lc160': 'Pointer-switch: both travel a+c+b steps and align on the shared tail.',
    '08:lc24':  'k-group reversal with k=2 — the arrow-order rehearsal.',
    '08:lc86':  'Two builder lists then stitch; sever the old tail link.',
    '08:lc61':  'Close into a ring, cut at n − k%n.',
    ...
    '09:lc496': 'The monotonic-stack hello-world: each arrival resolves all smaller waiters.',
    '09:lc225': 'One queue, rotate after push — the O(n) lives on the write side.',
    '09:lc735': 'A monotonic stack whose pop condition is a physics rule.',
    '09:lc394': 'Nesting = a stack of (count, prefix) call frames.',
    '09:lc71':  '“..” undoes the latest directory — textbook LIFO.',
    '09:lc227': '* and / bind to the previous term now; + and − defer as signed pushes.',
    ...
    '10:lc77':  'The start index forcing ascending picks = the sets-not-sequences switch.',
    '10:lc90':  'Sort + skip the same value at the same level (i > start).',
    '10:lc40':  'Combination Sum with i+1 (single use) plus the level-dedup line.',
    '10:lc93':  'Four segments of length 1–3; validation is the whole game.',
    '10:lc37':  'Choose/recurse/erase with a boolean threading “solved” up the stack.',
    ...
    '11:lc101': 'Recurse on pairs with crossed children: outer-outer, inner-inner.',
    '11:lc110': 'One post-order pass; −1 smuggles “unbalanced” up the stack.',
    '11:lc572': 'Outer walk picks the anchor, inner Same-Tree checks it.',
    '11:lc199': 'Level order keeping each level’s last node.',
    '11:lc105': 'Preorder hands out roots in order; inorder splits the spans.',
    '11:lc236': 'Post-order reports: the node where both sides answer is the LCA.',
    '11:lc124': 'Arch (both sides, record) vs segment (one side, return up).',
    ...
    '12:lc767': 'Most-plentiful-first with a one-slot cooldown (Task Scheduler, no idles).',
    '12:lc355': 'A news feed IS Merge k Sorted Lists, read from the tails.',
    '12:lc373': 'Heap frontier over the implicit sorted pair grid — touch only O(k) cells.',
    '12:lc502': 'Sorted unlock lane (capital) feeding a max-heap of wants (profit).',
    ...
    '13:lc720': '“Every prefix is a word” = every node on the trie path is flagged.',
    '13:lc421': 'A 32-bit number is a binary string — trie + opposite-bit greedy.',
    '13:lc1032':'Reverse the words: stream suffixes become trie prefixes.',
    ...
    '14:lc13':  'One rule: a symbol smaller than its right neighbor is subtractive.',
    '14:lc8':   'Spec discipline: spaces, one sign, digits, clamp inside the loop.',
    '14:lc38':  'Run-length encode the previous term, n−1 times.',
    '14:lc151': 'Carve words scanning backwards (or double-reversal in place).',
    '14:lc68':  'Greedy fill; spaces via div/mod with the remainder leftmost.',
    ...
    '15:lc463': 'No traversal: 4 per cell, −2 per shared edge, counted once.',
    '15:lc130': 'Solve the complement: flood from the border, flip the unmarked.',
    '15:lc909': 'BFS on square numbers; the 2D board is just a lookup table.',
    '15:lc127': 'Words are nodes, one-letter edits are edges — BFS the ladder.',
    ...
    '16:lc802': 'Three-color DFS: gray finds cycles, black memoizes safety.',
    '16:lc310': 'Peel leaves inward — Kahn’s algorithm converging on the tree’s center.',
    '16:lc1319':'Union-find counts components; redundant edges are the movable cables.',
    '16:lc332': 'Eulerian path via Hierholzer: post-order append, dead ends sink last.',
    ...
    '17:lc1514':'Dijkstra re-aimed: maximize a product with a max-heap.',
    '17:lc399': 'Division facts = weighted edges; answers = path products.',
    '17:lc1976':'Dijkstra + ways[]: strictly-better resets, ties accumulate.',
    ...
    '18:lc53':  'Kadane: best sum ENDING here — extend or restart.',
    '18:lc91':  'Climbing stairs with validity gates; the zeros are the exam.',
    '18:lc279': 'Coin Change with coins {1, 4, 9, 16, …}.',
    '18:lc740': 'Bucket by value, then House Robber along the value axis.',
    ...
    '19:lc63':  'Same recurrence — obstacles are just states pinned to zero.',
    '19:lc221': 'Side ending at each bottom-right corner: 1 + min of three supports.',
    '19:lc494': 'One line of algebra: P = (total+target)/2 → count subsets.',
    '19:lc115': 'The LCS table counting ways instead of maximizing.',
    ...
    '20:lc1049':'Smashing = signing: minimize |sum(A) − sum(B)| via subset sum.',
    '20:lc329': 'Strict increase kills cycles — the grid is a DAG, memo DFS is the DP.',
    '20:lc10':  '“x*” is the unit: delete it (j−2) or expand one copy (i−1, same j).',
    ...
    '21:lc45':  'BFS layers compressed to two ints: layer end, furthest reach.',
    '21:lc678': 'Carry the RANGE of possible open counts; “*” just widens it.',
    '21:lc846': 'The smallest remaining card is FORCED to lead a group (TreeMap).',
    '21:lc135': 'Split the two-sided constraint into two sweeps; max merges them.',
    ...
    '22:lc476': 'XOR with a same-length mask of 1s = flip only significant bits.',
    '22:lc260': 'XOR-all = a^b; split everyone on one differing bit, XOR each side.',
    '22:lc201': 'Only the endpoints’ common binary prefix survives a range AND.',
    ...
    '23:lc9':   'Reverse only half the digits — overflow becomes impossible.',
    '23:lc7':   'Check the overflow BEFORE the multiply (MAX/10 with digit 7/−8).',
    '23:lc43':  'Digit i × digit j lands at positions i+j and i+j+1.',
    '23:lc398': 'Reservoir sampling: keep match k with probability 1/k.',
    '23:lc149': 'Anchor a point; equal gcd-reduced slopes = same line.',
    ...
    '24:lc706': 'Separate chaining from scratch: hash → bucket → chain walk.',
    '24:lc622': 'Ring buffer: head + size dodges the full/empty ambiguity.',
    '24:lc1396':'Open-session map + per-route running (sum, count) aggregates.',
    '24:lc715': 'Disjoint TreeMap intervals; every op is floor/ceiling neighbor surgery.',
